...
abstract class Component implements IAccessibility {
    ...
    */
	...
	move(x: number, y: number): void {
		if (this.outerSvg != null) this.outerSvg.move(x, y);
		this.update();
	}
	public getState() {
		return this.state;
	...
	protected update(): void {
		...
		else throw new Error("Aria Role not implemented.");
	}
...
abstract class Widget extends Component {
	...
	protected registerWindowEvents(): void {
		(this.parent as Window).addObserver((state) => {
			...
			//the widget's bounds.
			if (state instanceof IdleUpWidgetState && this.getState() instanceof PressedOutWidgetState) {
				this.setState(new IdleUpWidgetState());
			}
...
// local
export { Window, Widget, Component, IAccessibility, RoleType, EventArgs, WidgetState };
export {
	IdleUpWidgetState,
	IdleDownWidgetState,
	HoverWidgetState,
	HoverPressedWidgetState,
	PressedWidgetState,
	PressedOutWidgetState,
	DragWindowState,
};
// from svg.js
